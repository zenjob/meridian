...
class SummarizerTest(parameterized.TestCase):
  ...
  def setUp(self):
    ...
    self.mock_calibra = mock.create_autospec(model.Calibra, instance=True)
    ...
    self.summarizer = summarizer.Summarizer(self.mock_calibra)
  ...
  def _stub_media_summary_plotters(self, media_summary):
    media_summary.plot_sales_contribution_waterfall_chart().to_json.return_value = (
        '{}'
    )
    media_summary.plot_sales_contribution_pie_chart().to_json.return_value = (
        '{}'
    )
  ...
  def _stub_for_insights(self):
    ...
    self.mock_calibra.n_rf_channels = 2

    # Stub `input_data.time` property.
    self.mock_calibra.input_data = mock.Mock()
    ...
    self.mock_calibra.input_data.time = response[c.TIME]
      ...
      (
          ...
          [
              ...
              (
                  ...
                  summary_text.OPTIMAL_FREQUENCY_CHART_DESCRIPTION,
              ),
          ],
      ),
  ])
  ...
  def test_sales_contrib_card_plotters_called(self):
    ...
    summary_html_dom = self._get_output_model_results_summary_html_dom()
    for mock_plot in [
        media_summary.plot_sales_contribution_waterfall_chart(),
        media_summary.plot_spend_vs_contribution(),
        media_summary.plot_sales_contribution_pie_chart(),
    ]:
  ...
  def test_optimal_analyst_card_insights_no_rf(self):
    self.mock_calibra.n_rf_channels = 0
    reach_frequency = self.reach_frequency
