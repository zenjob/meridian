...
class Summarizer:
  ...
  def _model_fit(self):
    return visualizer.ModelFit(self._calibra)

  @functools.cached_property
  def _model_diagnostics(self):
    return visualizer.ModelDiagnostics(self._calibra)
  ...
  def _response_time_values(self) -> Sequence[dt.datetime]:
    ...
    return [
        ...
        for value in self._calibra.input_data.time.values
    ]
  ...
  def _create_cards_htmls(
    ...
    media_summary = visualizer.MediaSummary(
        self._calibra, selected_times=selected_times
    )
  ...
  def _create_sales_contrib_card_html(
    ...
    channel_drivers_chart = formatter.ChartSpec(
        ...
        chart_json=media_summary.plot_sales_contribution_waterfall_chart().to_json(),
    )
    ...
    sales_contribution_chart = formatter.ChartSpec(
        ...
        chart_json=media_summary.plot_sales_contribution_pie_chart().to_json(),
    )

    lead_sales_channels = self._get_sorted_posterior_mean_metrics_df(
        media_summary, [c.INCREMENTAL_SALES]
    )[c.CHANNEL][:2]
  ...
  def _create_optimal_analyst_card_html(
    ...
    if reach_frequency is None:
      ...
      assert self._calibra.n_rf_channels > 0
