...
class Analyzer:
  """Runs calculations to analyze the raw data from fitting the model."""

  def __init__(self, calibra: model.Calibra):
    self._calibra = calibra
  ...
  def _get_adstock_dataframe(
    ...
    if channel_type is constants.MEDIA:
      prior = self._calibra.inference_data.prior.alpha_m.values[0]
      posterior = np.reshape(
          self._calibra.inference_data.posterior.alpha_m.values,
          (-1, self._calibra.n_media_channels),
      )
    else:
      prior = self._calibra.inference_data.prior.alpha_rf.values[0]
      posterior = np.reshape(
          self._calibra.inference_data.posterior.alpha_rf.values,
          (-1, self._calibra.n_rf_channels),
      )
  ...
  def _get_adstock_hill_tensors(
    ...
    adstock_tensors["media_scaled"] = (
        self._calibra.media_scaled
        if new_media is None or self._calibra.media_transformer is None
        else self._calibra.media_transformer.forward(new_media)
    )
    adstock_tensors["reach_scaled"] = (
        self._calibra.reach_scaled
        if new_reach is None or self._calibra.reach_transformer is None
        else self._calibra.reach_transformer.forward(new_reach)
    )
    adstock_tensors["frequency"] = (
        new_frequency if new_frequency is not None else self._calibra.frequency
    )
  ...
  def _get_adstock_hill_param_names(self) -> list[str]:
    ...
    """
    params = []
    if self._calibra.media is not None:
      params.extend([
          constants.EC_M,
          constants.SLOPE_M,
          constants.ALPHA_M,
    ...
    if self._calibra.reach is not None:
      params.extend([
          constants.EC_RF,
          constants.SLOPE_RF,
          constants.ALPHA_RF,
  ...
  def _get_transformed_media_and_beta(
    ...
    """
    if media is not None:
      media_transformed = self._calibra.adstock_hill_media(
          media=media,
          alpha=alpha_m,
          ec=ec_m,
          slope=slope_m,
    ...
    if reach is not None:
      rf_transformed = self._calibra.adstock_hill_rf(
          reach=reach,
          frequency=frequency,
          alpha=alpha_rf,
          ec=ec_rf,
  ...
  def filter_and_aggregate_geos_and_times(
    ...
    """Filter and aggregate geo and/or time dimensions of a tensor.
  ...
  def expected_impact(
    ...
    """Calculates either expected impact posterior or prior.
    ...
    other time dimensions (e.g. future predictions), but we do not allow that in
    this method because of the additional complexites this introduces:
    ...
    Args:
      ...
      new_reach: Optional tensor with dimensions matching reach.
      new_frequency: Optional tensor with dimensions matching frequency.
      new_controls: Optional tensor with dimensiosn matching controls.
      ...
      aggregate_geos: Boolean. If True, then the expected impact is summed over
        all regions.
      aggregate_times: Boolean. If True, then the expected impact is summed over
        all time periods.
      inverse_transform_impact: Boolean. If True, then the expected impact is
        returned in terms of the actual impact number (i.e., what is passed to
    ...
    Returns:
      ...
      n_times)`. The `n_geos` and `n_times` dimensions will be dropped if
    ...
    """
    if self._calibra.is_national:
      _warn_if_geo_arg_in_kwargs(
          aggregate_geos=aggregate_geos,
          selected_geos=selected_geos,
      )
    dist_type = constants.POSTERIOR if use_posterior else constants.PRIOR
    if dist_type not in self._calibra.inference_data.groups():
      raise model.NotFittedModelError(
          f"sample_{dist_type}() must be called prior to calling"
          " `expected_impact()`."
      )
    _check_shape_matches(
        new_controls, "new_controls", self._calibra.controls, "controls"
    )
    _check_shape_matches(new_media, "new_media", self._calibra.media, "media")
    _check_shape_matches(new_reach, "new_reach", self._calibra.reach, "reach")
    _check_shape_matches(
        new_frequency, "new_frequency", self._calibra.frequency, "frequency"
    )

    params = (
        self._calibra.inference_data.posterior
        if use_posterior
        else self._calibra.inference_data.prior
    )
    tensor_kwargs = self._get_adstock_hill_tensors(
        new_media, new_reach, new_frequency
    )
    tensor_kwargs["controls_scaled"] = (
        self._calibra.controls_scaled
        if new_controls is None
        else self._calibra.controls_transformer.forward(new_controls)
    )
    n_draws = params.draw.size
    n_chains = params.chain.size
    impact_means = tf.zeros(
        (n_chains, 0, self._calibra.n_geos, self._calibra.n_times)
    )
    ...
    if inverse_transform_impact:
      impact_means = self._calibra.kpi_transformer.inverse(impact_means)
      if not use_kpi:
        impact_means *= self._calibra.revenue_per_kpi
  ...
  def _get_incremental_kpi(
    ...
    """Function to compute incremental KPI on the unit scale.
    ...
    return tf.einsum(
        "...gtm,...gm->...gtm", combined_media_transformed, combined_beta
    )

  @tf.function(jit_compile=True)
  def _inverse_impact(
      self, incremental_impact: tf.Tensor, use_kpi: bool
  ) -> tf.Tensor:
    ...
    Args:
      incremental_impact: Tensor of incremenal impact modeled from parameter
        distributions.
    ...
    """
    t1 = self._calibra.kpi_transformer.inverse(
        tf.einsum("...m->m...", incremental_impact)
    )
    t2 = self._calibra.kpi_transformer.inverse(tf.zeros_like(t1))
  ...
  def incremental_impact(
    ...
    """Calculates either incremental impact posterior or prior.

    This calculates incremental impact for each posterior (or prior) parameter
    ...
    2) additive changes on the model kpi scale correspond to additive
      ...
      currently holds because the impact transformation only involves centering
      and scaling (no log transformations, for example).

    In principle, the incremental impact could be calculated
    with other time dimensions (e.g. future predictions), but we do not allow
    that in this method because of the additional complexites this introduces:
    1) corresponding price (revenue per KPI) data would also be needed,
    2) if the model contains weekly effect parameters, then some method is
      needed to estimate or predict these effects for time periods outside of
      the training data window.
    ...
    Args:
      ...
      aggregate_geos: Boolean. If True, then incremental impact is summed over
        all regions.
      aggregate_times: Boolean. If True, then incremental impact is summed over
        all time periods.
      inverse_transform_impact: Boolean. If True, the incremental impact is
        ...
        incremental impact is returned in terms of transformed impact (i.e.,
        what is modeled).
      use_kpi: Boolean. If True, the incremental KPI is calculated. If False,
        incremental revenue `(KPI * revenue_per_kpi)` is calculated. Only used
        if `inverse_transform_impact=True`.
      batch_size: Integer representing max draws per chain in each batch. The
        ...
        faster with larger `batch_size` values.
    ...
    Returns:
      ...
      RF channels. The `n_geos` and `n_times` dimensions will be
      dropped if `aggregate_geos=True` or `aggregate_times=True`, respectively.
    Raises:
      NotFittedModelError: if `sample_posterior()` (for `use_posterior=True`)
        or `sample_prior()` (for `use_posterior=False`) has not been called
        prior to calling this method.
    ...
    n_draws = params.draw.size
    n_chains = params.chain.size
    incremental_impact = tf.zeros((
        n_chains,
        0,
        self._calibra.n_geos,
        self._calibra.n_times,
        self._calibra.n_media_channels + self._calibra.n_rf_channels,
    ))
    batch_starting_indices = np.arange(n_draws, step=batch_size)
    param_list = self._get_adstock_hill_param_names()
    incremental_impact_temps = []
    for start_index in batch_starting_indices:
      stop_index = np.min([n_draws, start_index + batch_size])
      batch_dists = {
          k: tf.convert_to_tensor(params[k][:, start_index:stop_index, ...])
      ...
      incremental_impact_temps.append(
          self._get_incremental_kpi(**tensor_kwargs, **batch_dists)
      )
    incremental_impact = tf.concat(
        [incremental_impact, *incremental_impact_temps], axis=1
    )
    if inverse_transform_impact:
      incremental_impact = self._inverse_impact(incremental_impact, use_kpi)
  ...
  def _get_roi_tensors(
    ...
    Args:
      new_media: Optional. Media data, with the same shape as
        calibra.input.data.media, to be used to compute ROI for alternative
        media data. Default uses calibra.input.data.media.
      new_media_spend: Optional. Media_spend data, with the same shape as
        calibra.input.data.media_spend, to be used to compute ROI for
        alternative media_spend data. Default uses
        calibra.input.data.media_spend.
      new_reach: Optional. Reach data with the same shape as
        calibra.input.data.reach, to be used to compute ROI for alternative
        reach data. Default uses calibra.input.data.reach.
      new_frequency: Optional. Frequency data with the same shape as
        calibra.input.data.frequency, to be used to compute ROI for alternative
        frequency data. Default uses calibra.input.data.frequency.
      new_rf_spend: Optional. RF Spend data with the same shape as
        calibra.input.data.rf_spend, to be used to compute ROI for alternative
        rf_spend data. Default uses calibra.input.data.rf_spend.
    ...
    if selected_geos is not None or not aggregate_geos:
      if (
          self._calibra.media_spend is not None
          and not self._calibra.input_data.media_spend_has_geo_dimension
      ):
        raise ValueError(
      ...
      if (
          self._calibra.rf_spend is not None
          and not self._calibra.input_data.rf_spend_has_geo_dimension
      ):
        raise ValueError(
    ...
    if selected_times is not None or not aggregate_times:
      if (
          self._calibra.media_spend is not None
          and not self._calibra.input_data.media_spend_has_time_dimension
      ):
        raise ValueError(
      ...
      if (
          self._calibra.rf_spend is not None
          and not self._calibra.input_data.rf_spend_has_time_dimension
      ):
        raise ValueError(
            "aggregate_geos=False not allowed because Meridian rf_spend data"
    ...
    _check_shape_matches(
        new_media, constants.NEW_MEDIA, self._calibra.media, constants.MEDIA
    )
    _check_spend_shape_matches(
        new_media_spend,
        constants.NEW_MEDIA_SPEND,
        (
            tf.TensorShape((self._calibra.n_media_channels)),
            tf.TensorShape((
                self._calibra.n_geos,
                self._calibra.n_times,
                self._calibra.n_media_channels,
            )),
        ),
    )
    _check_shape_matches(
        new_reach, constants.NEW_REACH, self._calibra.reach, constants.REACH
    )
    _check_shape_matches(
        new_frequency,
        constants.NEW_FREQUENCY,
        self._calibra.frequency,
        constants.FREQUENCY,
    )
    _check_spend_shape_matches(
        new_rf_spend,
        ...
        (
            tf.TensorShape((self._calibra.n_rf_channels)),
            tf.TensorShape((
                self._calibra.n_geos,
                self._calibra.n_times,
                self._calibra.n_rf_channels,
            )),
        ),
    )

    media = self._calibra.media if new_media is None else new_media
    reach = self._calibra.reach if new_reach is None else new_reach
    frequency = (
        self._calibra.frequency if new_frequency is None else new_frequency
    )

    media_spend = (
        self._calibra.media_spend
        if new_media_spend is None
        else new_media_spend
    )
    rf_spend = self._calibra.rf_spend if new_rf_spend is None else new_rf_spend
  ...
  def marginal_roi(
    ...
    Marginal ROI numerator is the change in expected revenue (`kpi *
    revenue_per_kpi`) when one channel's spend is increased by a small fraction.
    Marginal ROI denominator is the corresponding small fraction of the
    ...
    Args:
      ...
      new_media: Optional. Media data with the same shape as
        ...
        media data. Default uses calibra.input.data.media.
      new_media_spend: Optional. Media_spend data with the same shape as
        calibra.input.data.spend, to be used to compute ROI for alternative
        media_spend data. Default uses calibra.input.data.media_spend.
      new_reach: Optional. Reach data with the same shape as
        calibra.input.data.reach, to be used to compute ROI for alternative
        reach data. Default uses calibra.input.data.reach.
      new_frequency: Optional. Frequency data with the same shape as
      ...
      aggregate_times: If `True`, then expected revenue is summed over all time
        periods.
      by_reach: For the channel w/ reach and frequency, return the mROI by reach
        given fixed frequency if true; return the mROI by frequency given fixed
        reach if false.
    ...
    Returns:
      Tensor of Marginal ROI values with dimensions `(n_chains, n_draws, n_geos,
      n_times, n_media_channels + n_rf_channels)`. The `n_geos` and `n_times`
      dimensions will be dropped if `aggregate_geos=True` or
  ...
  def roi(
    ...
    ROI numerator is the change in expected revenue `(kpi *
    revenue_per_kpi`) when one channel's spend is set to zero, leaving all other
    channels' spend unchanged. ROI denominator is the total spend of the
    channel.
    ...
    Args:
      ...
      new_media: Optional tensor with media to be used to compute ROI.
      new_media_spend: Optional tensor with media_spend to be used to compute
        ROI.
      new_reach: Optional tensor with reach to be used to compute ROI.
      new_frequency: Optional tensor with frequency to be used to compute ROI.
      new_rf_spend: Optional tensor with rf_spend to be used to compute ROI.
      selected_geos: Optional list containing a subset of geos to include. By
        default, all geos are included.
      selected_times: Optional list containing a subset of times to include. By
        default, all time periods are included.
      aggregate_geos: Boolean. If `True`, then expected revenue is summed over
        all regions.
      aggregate_times: Boolean. If `True`, then expected revenue is summed over
        all time periods.
      batch_size: Integer representing max draws per chain in each batch. The
        calculation is run in batches to avoid memory exhaustion. If a memory
        error occurs, try reducing `batch_size`. The calculation will generally
    ...
    Returns:
      ...
      n_media_channels + n_rf_channels)`. The `n_geos` and `n_times` dimensions
      will
      be dropped if `aggregate_geos=True` or `aggregate_times=True`,
      respectively.
    """
  ...
  def expected_vs_actual_data(
    ...
    """Calculates the data for the expected vs actual impact over time.
  ...
  def _compute_incremental_impact_aggregate(
    ...
    new_media = (
        tf.zeros_like(self._calibra.media)
        if self._calibra.media is not None
        else None
    )
    new_reach = (
        tf.zeros_like(self._calibra.reach)
        if self._calibra.reach is not None
        else None
    )
    new_frequency = (
        tf.zeros_like(self._calibra.frequency)
        if self._calibra.frequency is not None
        else None
    )
  ...
  def media_summary_metrics(
    ...
    Args:
      ...
      selected_times: Optional list containing a subset of times to include. By
        default, all time periods are included.
      aggregate_geos: Boolean. If `True`, then expected impact is summed over
        all regions.
      aggregate_times: Boolean. If `True`, then expected impact is summed over
        all time periods.
      batch_size: Integer representing max draws per chain in each batch. The
        calculation is run in batches to avoid memory exhaustion. If a memory
        error occurs, try reducing `batch_size`. The calculation will generally
    ...
    Returns:
      ...
      data variables: `pct_of_spend`, `spend`, `units`, `spend_per_unit`, `roi`,
      `mroi`, `incremental_impact`, `effectiveness`, `pct_of_actual_impact`,
      `pct_of_mean_expected_impact`.
    """
    dim_kwargs = {
        "selected_geos": selected_geos,
        "selected_times": selected_times,
    ...
    spend_list = []
    if self._calibra.n_media_channels > 0:
      spend_list.append(self._calibra.media_spend)
    if self._calibra.n_rf_channels > 0:
      spend_list.append(self._calibra.rf_spend)
    ...
    if self._calibra.n_media_channels > 0:
      units_list.append(self._calibra.media[:, -self._calibra.n_times :, :])

    if self._calibra.n_rf_channels > 0:
      units_list.append(
          self._calibra.reach[:, -self._calibra.n_times :, :]
          * self._calibra.frequency[:, -self._calibra.n_times :, :]
      )
    aggregated_units = self.filter_and_aggregate_geos_and_times(
        tensor=tf.concat(units_list, axis=-1), **dim_kwargs
    )
    units_with_total = tf.concat(
        [aggregated_units, tf.reduce_sum(aggregated_units, -1, keepdims=True)],
        axis=-1,
    )
    ...
    xr_coords = {
        constants.CHANNEL: (
            [constants.CHANNEL],
            list(self._calibra.input_data.get_all_channels())
            + [constants.ALL_CHANNELS],
        ),
    }
    if not aggregate_geos:
      geo_dims = (
          self._calibra.input_data.geo.data
    ...
    if not aggregate_times:
      time_dims = (
          self._calibra.input_data.time.data
    ...
    spend_data = self._compute_spend_data_aggregate(
        spend_with_total=spend_with_total,
        units_with_total=units_with_total,
        xr_dims=xr_dims,
        xr_coords=xr_coords,
    )
    ...
    pct_of_mean_expected_impact = self._compute_pct_of_mean_expected_impact(
        incremental_impact_prior=incremental_impact_prior,
        incremental_impact_posterior=incremental_impact_posterior,
    ...
    pct_of_actual_impact = self._compute_pct_of_actual_impact(
        incremental_impact_prior=incremental_impact_prior,
        incremental_impact_posterior=incremental_impact_posterior,
        xr_dims=xr_dims_with_ci_and_distribution,
        xr_coords=xr_coords_with_ci_and_distribution,
  ...
  def optimal_freq(
    ...
    """Calculates optimal frequency that maximizes posterior mean ROI.

    For this optimization, we restrict frequency to be constant across all
    ...
    remains unchanged as frequency varies.  We solve for the frequency at
    ...
    Args:
      ...
      use_posterior: Boolean. If True, posterior optimal frequencies are
        generated. Otherwise, prior optimal frequencies are generated.
      selected_geos: Optional list containing a subset of geos to include. By
        default, all geos are included.
      selected_times: Optional list containing a subset of times to include. By
        default, all time periods are included.
    ...
    Returns:
      XArray Dataset containing two variables: `optimal_frequency` and
        `roi_by_frequency`. `optimal_frequency` is the frequency which optimizes
    ...
    max_freq = np.max(np.array(self._calibra.frequency))
    if freq_grid is None:
      freq_grid = np.arange(1, max_freq, 0.1)
    roi = np.zeros(
        (len(freq_grid), self._calibra.n_rf_channels, 3)
    )  #  Last argument is 3 for the mean, lower and upper confidence intervals.

    for i, freq in enumerate(freq_grid):
      new_frequency = tf.ones_like(self._calibra.frequency) * freq
      new_reach = self._calibra.frequency * self._calibra.reach / new_frequency
      roi_temp = self.roi(
          new_reach=new_reach,
          new_frequency=new_frequency,
          use_posterior=use_posterior,
    ...
    rf_channel_values = (
        self._calibra.input_data.rf_channel.values
        if self._calibra.input_data.rf_channel is not None
        else []
    )
    return xr.Dataset(
        data_vars={
  ...
  def predictive_accuracy(
    ...
    `R-Squared`, `MAPE` and `wMAPE` are calculated on the KPI scale
    when revenue_per_kpi = None, or the revenue scale (KPI * revenue_per_kpi)
    ...
    Args:
      ...
      selected_times: Optional list containing a subset of dates to include. By
        default, all time periods are included.
      batch_size: Integer representing max draws per chain in each batch. By
        default, the batch_size is 100. The calculation is run in batches to
        avoid memory exhaustion. If a memory error occurs, try reducing
        `batch_size`. The calculation will generally be faster with larger
        `batch_size` values.

    Returns:
      A XArray Dataset containing the computed `R_Squared`, `MAPE` and `wMAPE`
      values, with coordinates `metric`, `geo_granularity` and `evaluation_set`,
      and accompanying data variable `value`. If holdout_id exists, the data is
      split into `Train`, `Test` and `All Data` subsections and the three
    ...
    actual = self.filter_and_aggregate_geos_and_times(
        tensor=self._calibra.kpi * self._calibra.revenue_per_kpi,
        **dims_kwargs,
    ).numpy()
    expected = np.mean(
    ...
    if self._calibra.model_spec.holdout_id is None:
      ...
      actual_train = np.where(
          self._calibra.model_spec.holdout_id, np.nan, actual
      )
      actual_test = np.where(
          self._calibra.model_spec.holdout_id, actual, np.nan
      )
      expected_train = np.where(
          self._calibra.model_spec.holdout_id, np.nan, expected
      )
      expected_test = np.where(
          self._calibra.model_spec.holdout_id, expected, np.nan
      )
      ...
      dataset = xr.Dataset(data_vars=xr_data, coords=xr_coords)
    if self._calibra.is_national and any(
  ...
  def _predictive_accuracy_helper(
    ...
    Args:
      actual_eval_set: Array with filtered and/or aggregated geo and time
        dimensions for the calibra.kpi * calibra.revenue_per_kpi calculation for
        either the `Train`, `Test`, or `All Data` evaluation sets.
  ...
  def _get_r_hat(self) -> Mapping[str, tf.Tensor]:
    ...
    Raises:
      NotFittedModelError: If self.sample_posterior() is not called before
        calling this method.
    """
    if constants.POSTERIOR not in self._calibra.inference_data.groups():
      raise model.NotFittedModelError(
    ...
    r_hat = tfp.mcmc.potential_scale_reduction({
        ...
        for k, v in self._calibra.inference_data.posterior.data_vars.items()
    })
    return r_hat
  ...
  def r_hat_summary(self, bad_r_hat_threshold: float = 1.2) -> pd.DataFrame:
    ...
    for param in r_hat:
      ...
      if self._calibra.prior_broadcast.has_determenistic_param(param):
        continue
  ...
  def response_curves(
    ...
    Args:
      ...
      confidence_level: Confidence level for prior and posterior credible
        intervals, represented as a value between zero and one.
      use_posterior: Boolean. If True, posterior response curves are generated.
        Otherwise, prior response curves are generated.
      selected_geos: Optional list containing a subset of geos to include. By
        default, all geos are included.
      selected_times: Optional list of containing a subset of time dimensions to
        ...
        strings/integers should align with the Calibra.n_times.
    ...
    Returns:
        XArray Dataset containing data needed to visualize response curves.
    """
    if self._calibra.is_national:
      _warn_if_geo_arg_in_kwargs(
          selected_geos=selected_geos,
    ...
    incremental_impact = np.zeros((
        ...
        len(self._calibra.input_data.get_all_channels()),
        3,
    ))
    for i, multiplier in enumerate(spend_multipliers):
      if multiplier == 0:
        incremental_impact[i, :, :] = tf.zeros(
            (len(self._calibra.input_data.get_all_channels()), 3)
    ...
    if self._calibra.n_media_channels > 0 and self._calibra.n_rf_channels > 0:
      spend = tf.concat(
          [self._calibra.media_spend, self._calibra.rf_spend], axis=-1
      )
    elif self._calibra.n_media_channels > 0:
      spend = self._calibra.media_spend
    else:
      spend = self._calibra.rf_spend
    ...
    xr_coords = {
        constants.CHANNEL: (
            [constants.CHANNEL],
            self._calibra.input_data.get_all_channels(),
        ),
        constants.METRIC: (
            [constants.METRIC],
            [constants.MEAN, constants.CI_LO, constants.CI_HI],
  ...
  def adstock_decay(self, confidence_level: float = 0.9) -> pd.DataFrame:
    """Calculates adstock decay for media and RF channels.
    ...
    Returns:
      Pandas DataFrame containing the channel, time_units, distribution, ci_hi,
      ci_lo, and mean for the Adstock function.
    """
    if (
        constants.PRIOR not in self._calibra.inference_data.groups()
        or constants.POSTERIOR not in self._calibra.inference_data.groups()
    ):
      raise model.NotFittedModelError(
    ...
    steps_per_time_period_max_lag = (
        ...
        // self._calibra.model_spec.max_lag
    )
    ...
    l_range = np.arange(0, self._calibra.model_spec.max_lag, step_size)
    ...
    media_channel_values = (
        self._calibra.input_data.media_channel.values
        if self._calibra.input_data.media_channel is not None
        else []
    )
  ...
  def _get_hill_curves_dataframe(
    ...
    if (
        ...
        and self._calibra.input_data.media_channel is not None
    ):
      ec = constants.EC_M
      slope = constants.SLOPE_M
      linspace = np.linspace(
          0,
          np.max(np.array(self._calibra.media_scaled), axis=(0, 1)),
          constants.HILL_NUM_STEPS,
      )
      channels = self._calibra.input_data.media_channel.values
    elif (
        channel_type == constants.RF
        and self._calibra.input_data.rf_channel is not None
    ):
      ec = constants.EC_RF
      slope = constants.SLOPE_RF
      linspace = np.linspace(
          0,
          np.max(np.array(self._calibra.frequency), axis=(0, 1)),
          constants.HILL_NUM_STEPS,
      )
      channels = self._calibra.input_data.rf_channel.values
    else:
      raise ValueError(
    ...
    hill_vals_prior = adstock_hill.HillTransformer(
        self._calibra.inference_data.prior[ec].values,
        self._calibra.inference_data.prior[slope].values,
    ).forward(expanded_linspace)[:, :, 0, :, :]
    hill_vals_posterior = adstock_hill.HillTransformer(
        self._calibra.inference_data.posterior[ec].values,
        self._calibra.inference_data.posterior[slope].values,
    ).forward(expanded_linspace)[:, :, 0, :, :]
    ...
    if channel_type == constants.MEDIA:
      media_transformers = transformers.MediaTransformer(
          self._calibra.media, self._calibra.population
      )
  ...
  def _get_hill_histogram_dataframe(self, n_bins: int) -> pd.DataFrame:
    ...
    """
    n_geos = self._calibra.n_geos
    n_media_times = self._calibra.n_media_times
    n_rf_channels = self._calibra.n_rf_channels
    n_media_channels = self._calibra.n_media_channels
    ...
    if self._calibra.input_data.rf_channel is not None:
      ...
      for i, channel in enumerate(self._calibra.input_data.rf_channel.values):
  ...
  def hill_curves(
    ...
    """Estimates Hill Curves table used for plotting each channel's curves.
  ...
  def _compute_marginal_roi_aggregate(
    ...
    incremented_tensors = _scale_tensors_by_multiplier(
        media=self._calibra.media,
        reach=self._calibra.reach,
        frequency=self._calibra.frequency,
  ...
  def _compute_spend_data_aggregate(
      self,
      spend_with_total: tf.Tensor,
      units_with_total: tf.Tensor,
      xr_dims: Sequence[str],
      xr_coords: Mapping[str, tuple[Sequence[str], Sequence[str]]],
  ) -> xr.Dataset:
    """Computes the parts of MediaSummary related to spend.
    ...
    """
    percentage_of_spend = (
        spend_with_total[..., :-1] / spend_with_total[..., -1:] * 100
    )
    percentage_of_spend_with_total = tf.concat(
        [
            percentage_of_spend,
            tf.reduce_sum(percentage_of_spend, -1, keepdims=True),
        ],
        axis=-1,
    )
    ...
    return xr.Dataset(
        data_vars={
            ...
            constants.PCT_OF_SPEND: (
                xr_dims,
                percentage_of_spend_with_total,
            ),
            constants.UNITS: (xr_dims, units_with_total),
            constants.SPEND_PER_UNIT: (
                xr_dims,
                spend_with_total / units_with_total,
            ),
        },
        coords=xr_coords,
    )
  ...
  def _compute_effectiveness_aggregate(
    ...
    return _mean_and_ci(
        prior=incremental_impact_prior / units_with_total,
        posterior=incremental_impact_posterior / units_with_total,
  ...
  def _compute_pct_of_mean_expected_impact(
      self,
      incremental_impact_prior: tf.Tensor,
      incremental_impact_posterior: tf.Tensor,
    ...
    return _mean_and_ci(
        ...
        * 100,
        metric_name=constants.PCT_OF_MEAN_EXPECTED_IMPACT,
        xr_dims=xr_dims,
        xr_coords=xr_coords,
        confidence_level=confidence_level,
    )

  def _compute_pct_of_actual_impact(
      self,
      incremental_impact_prior: tf.Tensor,
      incremental_impact_posterior: tf.Tensor,
      xr_dims: Sequence[str],
    ...
    """Computes the percentage of actual impact for MediaSummary metrics."""
    actual_impact = self.filter_and_aggregate_geos_and_times(
        tensor=self._calibra.kpi * self._calibra.revenue_per_kpi,
        **dim_kwargs,
    )
    return _mean_and_ci(
        prior=incremental_impact_prior / actual_impact[..., None] * 100,
        posterior=incremental_impact_posterior / actual_impact[..., None] * 100,
        metric_name=constants.PCT_OF_ACTUAL_IMPACT,
        xr_dims=xr_dims,
        xr_coords=xr_coords,
        confidence_level=confidence_level,
    )
