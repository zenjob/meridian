...
class ModelDiagnostics:
  ...
  def __init__(self, calibra: model.Calibra):
    ...
    self._analyzer = analyzer.Analyzer(calibra)
  ...
  def _predictive_accuracy_dataset(
    ...
    Args:
      selected_geos: Optional list of a subset of geo dimensions to include. By
        ...
        names from calibra.InputData. Only set one of `selected_geos` and
        `n_top_largest_geos`.
  ...
  def plot_prior_and_posterior_distribution(
    ...
    """
    if not (
        hasattr(self._calibra.inference_data, c.PRIOR)
        and hasattr(self._calibra.inference_data, c.POSTERIOR)
    ):
    ...
    if parameter not in self._calibra.inference_data.posterior.data_vars.keys():
      raise ValueError(
    ...
    if selected_times:
      param_data = self._calibra.inference_data.posterior[parameter]
    ...
    prior_dat = self._calibra.inference_data.prior[parameter]
    posterior_dat = self._calibra.inference_data.posterior[parameter]
    prior_df = (
    ...
    if c.GEO in prior_posterior_df.columns:
      top_geos = self._calibra.input_data.get_n_top_largest_geos(num_geos)
  ...
  def plot_rhat_boxplot(self) -> alt.Chart:
    ...
    """
    if not hasattr(self._calibra.inference_data, c.POSTERIOR):
      raise model.NotFittedModelError(
...
class ModelFit:
  ...
  Calculates the expected vs actual sales with the confidence level over time
  and plots graphs to compare the values.
  """

  def __init__(self, calibra: model.Calibra, confidence_level: float = 0.9):
    """Initializes the dataset based on the model and confidence level.

    Args:
      calibra: Media mix model with the raw data from the model fitting.
      confidence_level: Confidence level for expected sales credible intervals
  ...
  def model_fit_data(self) -> xr.Dataset:
    """Dataset holding the expected, actual, and baseline sales over time.

    The dataset contains the following:
    Coordinates:
      geo, time, metric (mean, ci_hi, ci_lo)
    Data variables:
      expected, baseline, actual (sales)
    """
  ...
  def plot_model_fit(
    ...
    """Plots the expected vs actual sales over time.
    ...
    Args:
      ...
      include_baseline: If True, shows the expected baseline sales without any
        media execution.
      include_ci: If True, shows the credible intervals for the expected sales.
    ...
    if n_top_largest_geos:
      selected_geos = self._calibra.input_data.get_n_top_largest_geos(
          n_top_largest_geos
      )
  ...
  def _validate_geos_to_plot(
    ...
    Args:
      selected_geos: Optional list of a subset of geo dimensions to include.
        Geos should match the geo dimension names from calibra.InputData. Only
  ...
  def _transform_data_to_dataframe(
    ...
    Args:
      ...
      include_baseline: If False, removes the expected baseline sales without
        any media execution.
...
class ReachAndFrequency:
  ...
  def __init__(
      self, calibra: model.Calibra, selected_times: Sequence[str] | None = None
  ):
...
class MediaEffects:
  ...
  Plots the effects of incremental sales and effectiveness for all channels.
  """
  ...
  def _transform_response_curve_metrics(
    ...
    Returns:
      ...
      channel, spend, spend_multiplier, ci_hi, ci_lo and incremental_sales
    """
    ...
    df = (
        self.response_curves_data[[c.SPEND, c.INCREMENTAL_SALES]]
...
class MediaSummary:
  ...
  (ROI, mROI, effectiveness) and media summary metrics (spend, impact, units).
  Metrics are calculated at the national level. These are used for various plots
  displaying these metrics.
  """
  ...
  def __init__(
    ...
    self._media_summary_metrics = self._analyzer.media_summary_metrics(
        confidence_level=confidence_level, selected_times=selected_times
    )
  ...
  def media_summary_metrics(self) -> xr.Dataset:
    ...
    Data variables:
      pct_of_spend, spend, units, spend_per_unit, roi, mroi, incremental_impact,
      effectiveness, pct_of_actual_impact, pct_of_mean_expected_impact
    """
  ...
  def update_media_summary_metrics(
    ...
    self._selected_times = selected_times
    self._media_summary_metrics = self._analyzer.media_summary_metrics(
        confidence_level=self._confidence_level, selected_times=selected_times
    )
  ...
  def plot_contribution_waterfall_chart(self) -> alt.Chart:
    ...
    pct = c.PCT_OF_MEAN_EXPECTED_IMPACT
    ...
    base = (
        alt.Chart(impact_df)
        .transform_window(
            sum_impact=f'sum({c.PCT_OF_MEAN_EXPECTED_IMPACT})',
            kwargs=f'lead({c.CHANNEL})',
        )
        .transform_calculate(
            prev_sum=(
                f'datum.sum_impact - datum.{c.PCT_OF_MEAN_EXPECTED_IMPACT}'
            ),
        )
        .encode(
            y=alt.Y(
  ...
  def plot_contribution_pie_chart(self) -> alt.Chart:
    ...
    base = alt.Chart(impact_df).encode(
        alt.Theta(f'{c.PCT_OF_MEAN_EXPECTED_IMPACT}:Q', stack=True),
        alt.Color(
            f'{c.CHANNEL}:N',
    ...
    text = base.mark_text(
        ...
        font=c.FONT_ROBOTO,
    ).encode(text=alt.Text(f'{c.PCT_OF_MEAN_EXPECTED_IMPACT}:Q', format='.0%'))
    return (
  ...
  def _transform_contribution_metrics(
    ...
    total_media_pct = (
        self.media_summary_metrics[c.PCT_OF_MEAN_EXPECTED_IMPACT]
        .sel(total_media_criteria)
        .item()
        / 100
    )
    ...
    baseline_df = pd.DataFrame(
        {
            ...
            c.PCT_OF_MEAN_EXPECTED_IMPACT: baseline_pct,
        },
        index=[0],
    )
    impact_df = self._media_summary_metrics_to_mean_df(
        metrics=[
            ...
            c.PCT_OF_MEAN_EXPECTED_IMPACT,
        ],
        selected_channels=selected_channels,
    )
    # Convert to percentage values between 0-1.
    impact_df[c.PCT_OF_MEAN_EXPECTED_IMPACT] = impact_df[
        c.PCT_OF_MEAN_EXPECTED_IMPACT
    ].div(100)
