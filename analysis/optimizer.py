...
class BudgetOptimizer:
  ...
  def __init__(self, calibra: model.Calibra):
    ...
    self._analyzer = analyzer.Analyzer(self._calibra)
  ...
  def optimize(
    ...
    Args:
      selected_times: Tuple containing the start and end time dimensions for the
        ...
        the Calibra time dims. By default, all times periods are used.
  ...
  def _get_selected_time_dims(
    ...
    """
    all_times = self._calibra.input_data.time.sortby(c.TIME)
  ...
  def _get_hist_spend(self, selected_times: Sequence[str]) -> np.ndarray:
    ...
    all_media = tf.convert_to_tensor(
        self._calibra.input_data.get_all_media_and_rf(), dtype=tf.float32
    )[:, -self._calibra.n_times :, :]
    all_spend = self._calibra.total_spend
  ...
  def _validate_pct_of_spend(
    ...
    if pct_of_spend is not None:
      if len(pct_of_spend) != len(self._calibra.input_data.get_all_channels()):
  ...
  def _validate_spend_constraints(
    ...
    if any(
        len(const) not in (1, len(self._calibra.input_data.get_all_channels()))
  ...
  def _get_incremental_impact_tensors(
    ...
    self._calibra.frequency otherwise. new_reach is calculated using
    ...
    Args:
      ...
      optimal_frequency: optimal frequency for rf channels in self._calibra.
    ...
    if self._calibra.n_media_channels > 0:
      new_media = (
          tf.math.divide_no_nan(
              spend[: self._calibra.n_media_channels],
              hist_spend[: self._calibra.n_media_channels],
          )
          * self._calibra.media
      )
    ...
    if self._calibra.n_rf_channels > 0:
      rf_media = self._calibra.reach * self._calibra.frequency
      new_rf_media = (
          tf.math.divide_no_nan(
              spend[-self._calibra.n_rf_channels :],
              hist_spend[-self._calibra.n_rf_channels :],
          )
          * rf_media
      )
      frequency = (
      ...
      new_rf_spend = tf.convert_to_tensor(spend[-self._calibra.n_rf_channels :])
    else:
      new_reach = None
      new_frequency = None
      new_rf_spend = None
  ...
  def _create_budget_dataset(
    ...
    all_times = self._calibra.input_data.time.values.tolist()
    return xr.Dataset(
        data_vars={
  ...
  def _update_incremental_impact_grid(
    ...
    Args:
      ...
      selected_times: Sequence of strings representing the time dimensions in
        `calibra.input_data.time` to use for optimization.
    ...
    if self._calibra.n_media_channels > 0:
      new_media = (
          multipliers_grid[i, : self._calibra.n_media_channels]
          * self._calibra.media
      )
    else:
      new_media = None
    ...
    if self._calibra.n_rf_channels == 0:
      ...
      new_frequency = tf.ones_like(self._calibra.frequency) * optimal_frequency
      new_reach = tf.math.divide_no_nan(
          multipliers_grid[i, -self._calibra.n_rf_channels :]
          * self._calibra.reach
          * self._calibra.frequency,
          new_frequency,
      )
    else:
      new_frequency = self._calibra.frequency
      new_reach = (
          multipliers_grid[i, -self._calibra.n_rf_channels :]
          * self._calibra.reach
      )
  ...
  def _create_grids(
    ...
    n_grid_columns = len(self._calibra.input_data.get_all_channels())
    ...
    if self._calibra.n_rf_channels > 0:
      rf_incremental_impact_max = np.nanmax(
          incremental_impact_grid[:, -self._calibra.n_rf_channels :], axis=0
      )
      rf_spend_max = np.nanmax(
          spend_grid[:, -self._calibra.n_rf_channels :], axis=0
      )
      rf_roi = tf.math.divide_no_nan(rf_incremental_impact_max, rf_spend_max)
      incremental_impact_grid[:, -self._calibra.n_rf_channels :] = (
          rf_roi * spend_grid[:, -self._calibra.n_rf_channels :]
      )
