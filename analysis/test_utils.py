...
SAMPLE_PCT_OF_MEAN_EXPECTED_IMPACT = np.array([
    [
        [1.579, 2.984],
        [0.363, 0.615],
        [3.379, 8.961],
...
SAMPLE_SPEND_PER_UNIT = np.array([
    2.0234682e-01,
    2.0265581e-01,
    2.0089923e-01,
    8.3189690e-05,
...
SAMPLE_PCT_OF_ACTUAL_IMPACT = np.array([
    [
        [3.08827233, 3.01591539],
        [0.71133897, 0.62209796],
        [6.60897124, 9.05605793],
    ],
    [
        [1.03005481, 5.12054253],
        [0.44672528, 0.54032602],
        [1.66529608, 14.99423728],
    ],
    [
        [2.2380681, 2.29379249],
        [0.47737934, 0.40516244],
        [4.86712303, 5.41813624],
    ],
    [
        [1.42115843, 2.29731059],
        [0.56882986, 0.36741987],
        [2.82361892, 7.85727348],
    ],
    [
        [2.50413799, 1.85282993],
        [0.22798578, 0.41736659],
        [7.76789248, 4.97165926],
    ],
    [
        [10.28168678, 14.58039856],
        [5.568382, 6.75502002],
        [16.8762208, 27.04932442],
    ],
])
...
def generate_media_summary_metrics() -> xr.Dataset:
  ...
  units = np.random.randint(low=10, high=1000, size=len(channel))
  spend_per_unit = np.random.random(size=len(channel))
  ...
  pct_of_actual_impact = np.random.randint(low=0, high=50, size=shape)
  pct_of_mean_expected_impact = np.random.randint(low=0, high=50, size=shape)

  return xr.Dataset(
      data_vars={
          c.PCT_OF_SPEND: ([c.CHANNEL], pct_of_spend),
          c.SPEND: ([c.CHANNEL], spend),
          c.UNITS: ([c.CHANNEL], units),
          c.SPEND_PER_UNIT: ([c.CHANNEL], spend_per_unit),
          c.ROI: (
...
def generate_response_curve_data(
  ...
  incremental_sales = np.random.lognormal(10, 1, size=shape)
