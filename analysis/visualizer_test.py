...
class ModelDiagnosticsTest(parameterized.TestCase):
  ...
  def setUpClass(cls):
    ...
    cls.calibra = mock.create_autospec(
        model.Calibra, instance=True, input_data=cls.input_data
    )
    ...
    type(cls.calibra).inference_data = mock.PropertyMock(
    ...
    cls.model_diagnostics = visualizer.ModelDiagnostics(cls.calibra)
  ...
  def test_transform_predictive_accuracy_no_holdout_id(
    ...
    model_diagnostics = visualizer.ModelDiagnostics(self.calibra)
  ...
  def test_transform_predictive_accuracy_holdout_id(
    ...
    calibra = mock.create_autospec(
        model.Calibra, instance=True, input_data=self.input_data
    )
    model_diagnostics = visualizer.ModelDiagnostics(calibra)
    df = model_diagnostics.predictive_accuracy_table(
        column_var=column_var,
    )
    self.assertEqual(list(df.columns), expected_columns)
  ...
  def test_plot_rhat_boxplot_mcmc_failed_raises_exception(self):
    ...
    type(calibra).inference_data = mock.PropertyMock(
        return_value=inference_data
    )
    model_diagnostics = visualizer.ModelDiagnostics(calibra)
...
class ModelFitTest(absltest.TestCase):
  ...
  def setUpClass(cls):
    ...
    cls.model_fit = visualizer.ModelFit(cls.calibra)
...
class ReachAndFrequencyTest(parameterized.TestCase):
  ...
  def setUpClass(cls):
    ...
    cls.reach_and_frequency = visualizer.ReachAndFrequency(cls.calibra)
  ...
  def test_reach_and_frequency_plot_optimal_freq_update_selected_times(self):
    ...
    reach_and_frequency = visualizer.ReachAndFrequency(
        self.calibra, selected_times=times1
    )
    self.mock_optimal_freq_method.assert_called_with(
        selected_times=times1
    )
...
class MediaEffectsTest(parameterized.TestCase):
  ...
  def setUpClass(cls):
    ...
    cls.media_effects = visualizer.MediaEffects(cls.calibra)
  ...
  def test_media_effects_plot_response_curves_update_selected_times(self):
    ...
    media_effects = visualizer.MediaEffects(
        self.calibra, confidence_level=0.9, selected_times=times
    )
    self.mock_response_curves_method.assert_called_with(
        spend_multipliers=list(np.arange(0, 2.2, c.RESPONSE_CURVE_STEP_SIZE)),
...
class MediaSummaryTest(parameterized.TestCase):
  ...
  def setUp(self):
    ...
    self.calibra = mock.create_autospec(
        model.Calibra, instance=True, input_data=self.input_data
    )
    ...
    self.media_summary = visualizer.MediaSummary(self.calibra)
  ...
  def test_media_summary_plot_waterfall_chart_correct_data(self):
    media_summary = visualizer.MediaSummary(self.calibra)
    ...
    self.assertFalse(
        any(pct > 1 or pct < 0 for pct in df.pct_of_mean_expected_impact)
    )
    self.assertEqual(
  ...
  def test_media_summary_plot_waterfall_chart_correct_formatted_text(
    ...
    media_metrics[c.PCT_OF_MEAN_EXPECTED_IMPACT].loc[total_media_dict] = pct
    with mock.patch.object(
        visualizer.MediaSummary,
        "media_summary_metrics",
        new=property(lambda unused_self: media_metrics),
  ...
  def test_media_summary_plot_pie_chart_correct_data(self):
    ...
    self.assertEqual(sum(plot.data[c.PCT_OF_MEAN_EXPECTED_IMPACT]), 1)
    self.assertSetEqual(
  ...
  def test_media_summary_plot_pie_chart_correct_encoding(self):
    ...
    self.assertEqual(
        arc_encoding,
        {
            ...
            "theta": {
                "field": c.PCT_OF_MEAN_EXPECTED_IMPACT,
                "stack": True,
                "type": "quantitative",
            },
        },
  ...
  def test_media_summary_plot_spend_vs_contribution_correct_data(self):
    ...
    media_metrics[c.PCT_OF_MEAN_EXPECTED_IMPACT].loc[total_media_dict] = 60
    with mock.patch.object(
        visualizer.MediaSummary,
        "media_summary_metrics",
        new=property(lambda unused_self: media_metrics),
