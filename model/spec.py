...
class ModelSpec:
  ...
  Attributes:
    ...
    holdout_id: Optional boolean tensor of dimensions (`n_geos` x `n_times`) or
      ...
      which are excluded from the training sample. Only sales data is excluded
      from the training sample - media data is still included as it may affect
      adstock of subsequent weeks. Note that if "ROI priors" are used (i.e.
      `use_roi_prior=True`), the `roi_m` parameters correspond to the ROI of all
      geos/times, even those in the holdout sample.
  """
