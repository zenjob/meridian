...
class ModelTest(tf.test.TestCase, parameterized.TestCase):
  ...
  def test_n_knots(self, knots, expected_n_knots):
    ...
    self.assertEqual(calibra._n_knots, expected_n_knots)
  ...
  def test_init_with_default_parameters_works(self):
    calibra = model.Calibra(input_data=self.input_data_with_media_only)

    # Compare input data.
    self.assertEqual(calibra.input_data, self.input_data_with_media_only)

    # Create sample model spec for comparison
    sample_spec = spec.ModelSpec()

    # Compare model spec.
    self.assertEqual(calibra.model_spec, sample_spec)

  def test_init_with_default_national_parameters_works(self):
    calibra = model.Calibra(input_data=self.national_input_data)

    # Compare input data.
    self.assertEqual(calibra.input_data, self.national_input_data)

    # Create sample model spec for comparison
    expected_spec = spec.ModelSpec()

    # Compare model spec.
    self.assertEqual(calibra.model_spec, expected_spec)
  ...
  def test_base_geo_properties(self):
    calibra = model.Calibra(input_data=self.input_data_with_media_and_rf)
    self.assertEqual(calibra.n_geos, self._N_GEOS)
    self.assertEqual(calibra.n_controls, self._N_CONTROLS)
    self.assertEqual(calibra.n_times, self._N_TIMES)
    self.assertEqual(calibra.n_media_times, self._N_MEDIA_TIMES)
    self.assertFalse(calibra.is_national)
    self.assertIsNotNone(calibra.prior_broadcast)
    self.assertIsNotNone(calibra.inference_data)
    self.assertNotIn(constants.PRIOR, calibra.inference_data.attrs)
    self.assertNotIn(constants.POSTERIOR, calibra.inference_data.attrs)

  def test_base_national_properties(self):
    calibra = model.Calibra(input_data=self.national_input_data)
    self.assertEqual(calibra.n_geos, self._N_GEOS_NATIONAL)
    self.assertEqual(calibra.n_controls, self._N_CONTROLS)
    self.assertEqual(calibra.n_times, self._N_TIMES)
    self.assertEqual(calibra.n_media_times, self._N_MEDIA_TIMES)
    self.assertTrue(calibra.is_national)
    self.assertIsNotNone(calibra.prior_broadcast)
    self.assertIsNotNone(calibra.inference_data)
    self.assertNotIn(constants.PRIOR, calibra.inference_data.attrs)
    self.assertNotIn(constants.POSTERIOR, calibra.inference_data.attrs)
  ...
  def test_input_data_tensor_properties(self, input_data):
    calibra = model.Calibra(input_data=input_data)
    self.assertAllEqual(
  ...
  def test_media_effects_dist_property(
    ...
    self.assertEqual(calibra.media_effects_dist, expected_media_effects_dist)
  ...
  def test_unique_sigma_for_each_geo_property(
    ...
    self.assertEqual(
        calibra.unique_sigma_for_each_geo, expected_unique_sigma_for_each_geo
    )
  ...
  def test_broadcast_prior_distribution_is_called_in_calibra_init(self):
    calibra = model.Calibra(input_data=self.input_data_with_media_and_rf)
    # Validate `tau_g_excl_baseline` distribution.
    self.assertEqual(
        calibra.prior_broadcast.tau_g_excl_baseline.batch_shape,
        (calibra.n_geos - 1,),
    )

    # Validate `n_knots` shape distributions.
    self.assertEqual(
        calibra.prior_broadcast.knot_values.batch_shape, (calibra._n_knots,)
    )

    # Validate `n_media_channels` shape distributions.
    n_media_channels_distributions_list = [
        calibra.prior_broadcast.beta_m,
        calibra.prior_broadcast.eta_m,
        calibra.prior_broadcast.alpha_m,
        calibra.prior_broadcast.ec_m,
        calibra.prior_broadcast.slope_m,
        calibra.prior_broadcast.roi_m,
    ]
    for broad in n_media_channels_distributions_list:
      self.assertEqual(broad.batch_shape, (calibra.n_media_channels,))

    # Validate `n_rf_channels` shape distributions.
    n_rf_channels_distributions_list = [
        calibra.prior_broadcast.beta_rf,
        calibra.prior_broadcast.eta_rf,
        calibra.prior_broadcast.alpha_rf,
        calibra.prior_broadcast.ec_rf,
        calibra.prior_broadcast.slope_rf,
        calibra.prior_broadcast.roi_rf,
    ]
    for broad in n_rf_channels_distributions_list:
      self.assertEqual(broad.batch_shape, (calibra.n_rf_channels,))

    # Validate `n_controls` shape distributions.
    n_controls_distributions_list = [
        calibra.prior_broadcast.gamma_c,
        calibra.prior_broadcast.xi_c,
    ]
    for broad in n_controls_distributions_list:
      self.assertEqual(broad.batch_shape, (calibra.n_controls,))

    # Validate sigma.
    self.assertEqual(calibra.prior_broadcast.sigma.batch_shape, (1,))

  def test_media_attributes_not_set(self):
    calibra = model.Calibra(input_data=self.input_data_with_rf_only)
    self.assertEqual(calibra.n_media_channels, 0)
    self.assertIsNone(calibra.media_transformer)
    self.assertIsNone(calibra.media_scaled)
    self.assertIsNone(calibra._media_counterfactual)
    self.assertIsNone(calibra._media_counterfactual_scaled)
    self.assertIsNone(calibra._media_spend_counterfactual)

  def test_rf_attributes_not_set(self):
    calibra = model.Calibra(input_data=self.input_data_with_media_only)
    self.assertEqual(calibra.n_rf_channels, 0)
    self.assertIsNone(calibra.reach_transformer)
    self.assertIsNone(calibra.reach_scaled)
    self.assertIsNone(calibra._reach_counterfactual)
    self.assertIsNone(calibra._reach_counterfactual_scaled)
    self.assertIsNone(calibra._rf_spend_counterfactual)

  def test_scaled_data_shape(self):
    calibra = model.Calibra(input_data=self.input_data_with_media_and_rf)
    if (
        self.input_data_with_media_and_rf.media is not None
        and calibra.media_scaled is not None
    ):
      self.assertAllEqual(
          calibra.media_scaled.shape,
    ...
    if (
        ...
        and calibra.reach_scaled is not None
    ):
      self.assertAllEqual(
          calibra.reach_scaled.shape,
  ...
  def test_scaled_data_inverse_is_identity(self):
    ...
    if (
        calibra.media_scaled is not None
        and calibra.media_transformer is not None
        and self.input_data_with_media_and_rf.media is not None
    ):
      self.assertAllClose(
          calibra.media_transformer.inverse(calibra.media_scaled),
          self.input_data_with_media_and_rf.media,
          atol=atol,
      )
    if (
        calibra.reach_scaled is not None
        and calibra.reach_transformer is not None
        and self.input_data_with_media_and_rf.reach is not None
    ):
      self.assertAllClose(
          calibra.reach_transformer.inverse(calibra.reach_scaled),
          self.input_data_with_media_and_rf.reach,
          atol=atol,
      )
    self.assertAllClose(
        calibra.controls_transformer.inverse(calibra.controls_scaled),
        self.input_data_with_media_and_rf.controls,
        atol=atol,
    )
    self.assertAllClose(
        calibra.kpi_transformer.inverse(calibra.kpi_scaled),
  ...
  def test_counterfactual_data_no_roi_calibration(self):
    ...
    self.assertAllEqual(
        calibra._media_counterfactual,
        tf.zeros_like(self.input_data_with_media_and_rf.media),
    )
    self.assertAllEqual(
        calibra._media_counterfactual_scaled,
        tf.zeros_like(calibra.media_scaled),
    )
    self.assertAllEqual(
        calibra._media_spend_counterfactual,
        tf.zeros_like(self.input_data_with_media_and_rf.media_spend),
    )
    if calibra.reach_scaled is not None:
      self.assertAllEqual(
          calibra._reach_counterfactual,
          tf.zeros_like(self.input_data_with_media_and_rf.reach),
      )
      self.assertAllEqual(
          calibra._reach_counterfactual_scaled,
          tf.zeros_like(calibra.reach_scaled),
      )
      self.assertAllEqual(
          calibra._rf_spend_counterfactual,
  ...
  def test_counterfactual_data_with_roi_calibration(
    ...
    self.assertAllClose(
        calibra._media_counterfactual_scaled,
        tf.where(roi_calibration_period, 0, calibra.media_scaled),
    )

    self.assertAllClose(
        calibra._media_spend_counterfactual,
        tf.where(
            roi_calibration_period[..., -calibra.n_times :, :],
            0,
            calibra.media_spend,
        ),
    )
  ...
  def test_counterfactual_data_with_rf_roi_calibration(
    ...
    if calibra.input_data.reach is not None:
      self.assertAllClose(
          calibra._reach_counterfactual,
          tf.where(
      ...
      self.assertAllClose(
          calibra._reach_counterfactual_scaled,
          tf.where(rf_roi_calibration_period, 0, calibra.reach_scaled),
      )

      self.assertAllClose(
          calibra._rf_spend_counterfactual,
          tf.where(
              rf_roi_calibration_period[..., -calibra.n_times :, :],
              0,
              calibra.rf_spend,
          ),
      )
  ...
  def test_baseline_geo_idx(
    ...
    self.assertEqual(calibra._baseline_geo_idx, expected_idx)
  ...
  def test_adstock_hill_rf(
    ...
    calibra.adstock_hill_rf(
        reach=calibra.reach,
        frequency=calibra.frequency,
  ...
  def test_get_joint_dist_zeros(self):
    ...
    sample = calibra._get_joint_dist_unpinned().sample(self._N_DRAWS)
    self.assertAllEqual(
        sample.y,
  ...
  def test_get_joint_dist_with_log_prob_media_only(
    ...
    par_structtuple = calibra._get_joint_dist_unpinned().sample(1)
    par = par_structtuple._asdict()

    # Note that "y" is a draw from the prior predictive (transformed) sales
    # distribution. We drop it because "y" is already "pinned" in
    # calibra._get_joint_dist() and is not actually a parameter.
    del par["y"]

    # Note that the actual (transformed) sales data is "pinned" as "y".
    log_prob_parts_structtuple = calibra._get_joint_dist().log_prob_parts(par)
    log_prob_parts = {
    ...
    for parname in prior_distribution_params:
      prior_distribution_logprobs[parname] = tf.reduce_sum(
          getattr(calibra.prior_broadcast, parname).log_prob(par[parname])
      )
      self.assertAllClose(
    ...
    transformed_media = calibra.adstock_hill_media(
        media=calibra.media_scaled,
    ...
    tau_g_logprob = tf.reduce_sum(
        getattr(
            calibra.prior_broadcast, constants.TAU_G_EXCL_BASELINE
    ...
    self.assertAllClose(
        posterior_unnormalized_logprob,
        calibra._get_joint_dist().log_prob(par)[0],
    )
  ...
  def test_get_joint_dist_with_log_prob_rf_only(
    ...
    transformed_reach = calibra.adstock_hill_rf(
        reach=calibra.reach_scaled,
        frequency=calibra.frequency,
  ...
  def test_sample_prior_media_and_rf_returns_correct_shape(self):
    ...
    calibra.sample_prior(n_draws=self._N_DRAWS)
    ...
    sigma_shape = (
        ...
        if calibra.unique_sigma_for_each_geo
    ...
    prior = calibra.inference_data.prior
    shape_to_params = {
        knots_shape: [
  ...
  def test_sample_posterior_media_and_rf_returns_correct_shape(self):
    ...
    posterior = calibra.inference_data.posterior
    shape_to_params = {
        knots_shape: [
    ...
    for attr in [
      ...
      self.assertEqual(
          getattr(calibra.inference_data.sample_stats, attr).shape,
          (
              self._N_CHAINS,
              self._N_KEEP,
          ),
    ...
    for attr in [
      ...
      self.assertEqual(
          getattr(calibra.inference_data.trace, attr).shape,
          (
              self._N_CHAINS,
              self._N_KEEP,
          ),
  ...
  def test_save_and_load_works(self):
    ...
    mmm = model.Calibra(input_data=self.input_data_with_media_and_rf)
