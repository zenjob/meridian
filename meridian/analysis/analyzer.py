...
class Analyzer:
  ...
  def roi(
    ...
    It is not recommended to set `aggregate_times=False`. When
    `aggregate_times=False`, the ROI output of this method is defined as the
    incremental outcome generated during the time period divided by the spend
    during that time period. The numerator is NOT the incremental outcome
    generated by media spend in the denominator. (The numerator includes the
    effect of past media execution on the current time period. The numerator
    does not include the effect of the current time period's media execution on
    the future.)

    The ROI defined by this method is just one of several ways that the ROI of a
    single time period could be defined. Alternative definitions could better
    align the ROI numerator and denominator; however, such definitions are not
    unique. The incremental outcome generated by a single time period of spend
    depends on the spend in both previous and subsequent weeks. For example, the
    incremental outcome could be defined to include the impact "on top of" or
    "in the absence of" the spend of previous and subsequent weeks.

    When `aggregate_times=True` and `selected_times` includes a reasonably large
    number of time periods, then there is better alignment between the ROI
    numerator and denominator. This is the recommended usage.
  ...
  def summary_metrics(
    ...
    mroi = self._compute_marginal_roi_aggregate(
        ...
        **batched_kwargs,
        ...
        # aggregated "All Paid Channels" channel dimension value. "Marginal ROI"
        # calculation must arbitrarily assume how the "next dollar" of spend
        # is allocated across "All Paid Channels" in this case, which may cause
